...
import { Crumbs, DataOrigin, Disclosure, Empty, EntitySection, Explain, Facts, FitBadge, Glance, LicenseShort, OriginBasis, SectionNav, Speed, Tag } from '@/components/ui';
import { CapabilityDetail, EntityMark, EstimateTag, MemoryScale, SystemsMeter } from '@/components/viz';
...
const SECTIONS = [
  { id: 'compatibility', label: 'Where it runs' },
  { id: 'variants', label: 'Versions & downloads' },
  { id: 'benchmarks', label: 'Benchmarks' },
  { id: 'community', label: 'Community' },
  { id: 'details', label: 'Details & sources' },
];
    ...
    <>
      ...
      <div className="entity-top">
        <header className="entity-hero">
          ...
          <Glance
            items={[
              ...
              ['Memory to run', listItem?.minMemoryGb != null ? <span key="m">~{Math.ceil(listItem.minMemoryGb)} GB <EstimateTag /></span> : '—', 'smallest, 8K ctx'],
        ...
        <aside className="verdict" aria-label="At a glance">
          <div>
            <h2>Can I run it? <span><SystemsMeter runsWell={runsWell.length} slow={slowly.length} of={systems.length} /> <EstimateTag title="Whether it fits is estimated from file size, quantization and context — not measured" /></span></h2>
      ...
      <EntitySection id="compatibility" title="Where it runs" intro={<>Best-fitting version on each reference system, at an 8K <Explain term="context" />.</>} more={<Link href="/run">Your hardware →</Link>}>
        ...
        <h3 className="subhead" id="performance">Measured throughput</h3>
        <PerformanceTable results={model.performanceResults} />
      </EntitySection>

      <EntitySection id="variants" title="Versions & downloads" intro={<>Each version is a separate set of weights. Expand one for its downloads: <Explain term="quantization" /> trades a little quality for a much smaller file, and the memory column adds the <Explain term="kv-cache" /> a conversation needs on top.</>}>
        {model.summary && <p className="small muted" style={{ margin: '0 0 12px' }}>Note: {model.summary}</p>}
        {model.variants.map((v, i) => {
          ...
          return (
            <Disclosure
              ...
              meta={<>{VARIANT_KIND_EXPLAINER[v.kind] ?? humanize(v.kind)} <span className="faint">· {v.publisher.name}{thirdParty ? ' (third-party)' : ''} · {plural(v.artifacts.length, 'download')}</span></>}
              aside={<LicenseShort commercialUse={v.license?.commercialUse} />}
            >
              <div className="split-wide">
                <dl className="kv">
                  <dt>Good for</dt><dd><div className="tags">{v.capabilities.filter((c) => c !== 'multilingual').map((c) => <Tag key={c}>{CAPABILITY_LABEL[c] ?? humanize(c)}</Tag>)}</div></dd>
                      ...
                      {v.artifacts.map((a) => {
                        ...
                        return (
                          <tr key={a.slug} id={a.slug}>
                            ...
                            <td><MemoryScale gb={total} label="" showValue={false} ticks={[8, 24, 96]} /><span className="val-estimated small">~{total.toFixed(1)} GiB</span> <EstimateTag /></td>
        ...
        {model.siblings.length > 0 && (
          <p className="small muted" style={{ marginTop: 12 }}>Other sizes in {model.release.name}: {model.siblings.map((s, i) => <span key={s.slug}>{i > 0 && ', '}<Link className="link" href={`/models/${s.slug}`}>{s.name}</Link></span>)}</p>
        )}
      </EntitySection>

      <EntitySection id="benchmarks" title="Benchmarks" intro={<>Each <Explain term="benchmark" /> score is labelled with who produced it. Developer-reported scores use each lab’s own prompts and settings; a benchmark that runs every model itself uses one setup for all. Bars are relative to the best open result.</>}>
        {benchSlugs.length === 0 ? <Empty>No benchmark results recorded for this model.</Empty> : (
          <div className="table-wrap">
            <table className="data bench">
              <thead>
            ...
            <p className="small muted" style={{ marginTop: 6 }}>
              <OriginBasis origins={benchmarkOrigins} />
      ...
      <EntitySection id="community" title="Community" intro="Benchmark runs and reviews from people running it on their own hardware.">
        <div className="split">
          <div id="community-runs">
            <h3 className="subhead" style={{ marginTop: 0, display: 'flex', justifyContent: 'space-between' }}>Benchmark runs <IfContributing><Link className="link" style={{ textTransform: 'none', letterSpacing: 0, fontWeight: 500 }} href={`/contribute/benchmark?model=${model.slug}`}>Submit a run →</Link></IfContributing></h3>
            <SubmissionList submissions={shownSubmissions} emptyText={hideCommunity ? SAMPLE_EMPTY_TEXT : undefined} />
          </div>
          <div id="reviews">
            <h3 className="subhead" style={{ marginTop: 0 }}>Reviews</h3>
            <ReviewList reviews={shownReviews} showSubject emptyText={hideCommunity ? SAMPLE_EMPTY_TEXT : 'No reviews for this model yet.'} />
          </div>
        </div>
      </EntitySection>

      <EntitySection id="details" title="Details & sources">
        <div className="split">
          <div>
            <h3 className="subhead" style={{ marginTop: 0 }}>Architecture</h3>
            <Facts
              items={[
            ...
            />
            <h3 className="subhead" id="lineage">Lineage</h3>
            <p className="small muted" style={{ margin: '0 0 8px' }}>How this model’s versions relate to each other and to other models.</p>
